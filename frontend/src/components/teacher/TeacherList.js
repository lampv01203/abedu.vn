  ...
  const renderRows = () => {
    return filteredTeachers.map((teacher, index) => (
      <tr key={teacher.teacher_id}>
        ...
        <td>
          <Link to={`/editTeacher/${teacher.teacher_id}`} className="text-primary">
            {teacher.full_name}
          </Link>
        </td>
        <td className="w-center">
          {format(new Date(teacher.birthday), "dd/MM/yyyy")}
        </td>
        <td>{teacher.phone}</td>
        <td>{teacher.facebook}</td>
        <td className="w-center">{teacher.department_code}</td>

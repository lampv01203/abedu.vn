...
import TotalTeacherAttend from "./TotalTeacherAttend"; // Import modal
...
const TeacherClassList = ({ teacherId }) => {
  ...
  const [selectedTeacherId, setSelectedTeacherId] = useState(teacherId); // Lưu teacherId
  ...
  const renderRows = () => {
    return filteredClasses.map((classItem, index) => (
      <tr key={classItem.class_id} onClick={() => handleClassClick(classItem.class_id, classItem.class_name)}>
    ...
    <div className="card max-h-354">
      ...
      <Modal
        ...
        <Modal.Header className="bg-primary text-white" closeButton>
          <Modal.Title>Thông tin chi tiết đứng lớp {selectedClassName}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedClassId && (
            <TotalTeacherAttend
